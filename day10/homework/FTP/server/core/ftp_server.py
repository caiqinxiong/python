...
class FtpServer(socketserver.BaseRequestHandler):
    ...
    def getFile(self,opt_dict):
        ...
        if not os.path.exists(opt_dict['file_path']): #判断服务器上是否存在该文件
            ...
            opt_dict['filesize'] = os.path.getsize(opt_dict['file_path']) # 获取文件总字节大小
            opt_dict['filename'] = os.path.basename(opt_dict['file_path']) # 获取文件名
        dic_b = json.dumps(opt_dict).encode('utf-8')
        cn.mySend(self.request,dic_b) # 将文件基本信息反馈给客户端
        if opt_dict['flag']:cn.getFile(self.request,opt_dict) # 文件存在，开始发送文件给客户端


    def putFile(self,opt_dict):
        '''客户端上传文件到服务器'''
        opt_dict['filesize'] = os.path.getsize(opt_dict['file_path']) # 获取文件大小
        opt_dict = cn.checkQuota(ss.USER_FILE,opt_dict) # 校验用户在服务器的磁盘配额
        log.readAndWrite(opt_dict['msg'])
        dic_b = json.dumps(opt_dict).encode('utf-8')
        cn.mySend(self.request,dic_b) # 将文件基本信息反馈给客户端
        if opt_dict['flag']:cn.putFile(self.request,opt_dict) # 开始接收客户端上传的文件
